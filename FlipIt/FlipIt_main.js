...
import {FlipIt_Board,showMoves} from "./FlipIt_Board.js";
import {FlipIt_Display} from "./FlipIt_Display.js";
...
function play(no,state){
    ...
    board = new FlipIt_Board(no,state,display);
}
...
function initEventHandlers() {
    ...
    $("input[name=state-no]:first").prop("checked",true); // check first state
        ...
        $("#solve").on("mousedown",()=>
            solveWeb(problemNo,startStates[problemNo],FlipIt_Board,display,showMoves));
        $("#reset").append(
...
$(document).ready(function() {
    ...
    $("body").load("../body.html",function(){
        $("head title, .title").text("Flip It");
        $("#svg_element").attr("viewBox","0 0 4 4").attr("width","400px");
        setSVGfactors();
        initLanguageHandlers();
        initEventHandlers();
        display = new FlipIt_Display();
        play(problemNo,startStates[problemNo]);
    })
});