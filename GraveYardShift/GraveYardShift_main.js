...
import {GraveYardShift_Display} from "./GraveYardShift_Display.js";
...
function play(no,state){
    ...
    board = new GraveYardShift_Board(no,state,display);
}
...
function initEventHandlers() {
    ...
    $("input[name=state-no]:first").prop("checked",true); // check first state
        ...
        $("#solve").on("mousedown",()=>
            solveWeb(problemNo,startStates[problemNo],GraveYardShift_Board,display,showMoves));
        $("#reset").append(
...
$(document).ready(function() {
    ...
    $("body").load("../body.html",function(){
        $("head title, .title[lang='en']").text("GraveYard Shift");
        $(".title[lang='fr").text("Tombe Frayeur")
        $("#svg_element").attr("viewBox","-0.5 -0.5 5 4").attr("width","500px")
        setSVGfactors();
        initLanguageHandlers();
        initEventHandlers();
        display = new GraveYardShift_Display();
        play(problemNo,startStates[problemNo]);
    })
});