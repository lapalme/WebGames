...
import {GrizzlyGears_Display} from "./GrizzlyGears_Display.js";
...
function play(no,state){
    ...
    board = new GrizzlyGears_Board(no,state,display);
}
...
function initEventHandlers() {
    ...
    $("input[name=state-no]:first").prop("checked",true); // check first state
        ...
        $("#solve").on("mousedown",()=>
            solveWeb(problemNo,startStates[problemNo],GrizzlyGears_Board,display,showMoves));
        $("#reset").append(
...
$(document).ready(function() {
    ...
    $("body").load("../body.html",function(){
        $("head title, .title[lang='en']").text("GrizzlyGears");
        $(".title[lang='fr").text("Parc'Ours en Forêt")
        $("#svg_element").attr("viewBox","-1.5 -1 6 5").attr("width","600px");  
        setSVGfactors();
        initLanguageHandlers();
        initEventHandlers();
        display = new GrizzlyGears_Display();
        play(problemNo,startStates[problemNo]);
    })
});